// src/utils/formatter.ts - Complete improved version for large projects

import path from "path";
import { filesize } from "filesize";
import chalk from "chalk";
import { ProgramOptions, FileStat } from "../types";
import { getFileTree, isLikelyBinaryFile } from "./file-utils";
...
function formatFileContent(
  ...
  if (content.length > maxFileSize) {
    if (options.truncateLargeFiles) {
      ...
      // Just mention that the file was skipped
      return "// File too large to include. Showing file information only.";
    }
  }
